...
def comp_choice(player_ch):
    ...
    # вывод результата
    if winner == win[0]:
        res = tk.Label(window, text = winner, width = w, height = h, bg = "white", fg = "green").grid(row = 0, column = 1)
    elif winner == win[1]:
        res = tk.Label(window, text = winner, width = w, height = h, bg = "white", fg = "red").grid(row = 0, column = 1)
    else:
        res = tk.Label(window, text = winner, width = w, height = h, bg = "white", fg = "black").grid(row = 0, column = 1)
    ...
    if winner == win[0]:
        comments = tk.Label(window, text = reswin, width = w, height = 1).grid(row = 1, column = 1)
    elif winner == win[1]:
        comments = tk.Label(window, text = reslose, width = w, height = 1).grid(row = 1, column = 1)
    else:
        comments = tk.Label(window, text = resdraw, width = w, height = 1).grid(row = 1, column = 1)
...
res = tk.Label(window, text = "Кто победит?", width = w, height = h, bg = "white", fg = "black").grid(row = 0, column = 1)
comments = tk.Label(window, text = "Что же ждёт нас сегодня?", width = w, height = 1).grid(row = 1, column = 1)
...
#кнопки выбора
btn_rock = tk.Button(text = "Камень", width = w, height = h, bg = "white", fg = "black", command = rock).grid(row = 4, column = 0)
...
button = tk.Button(text = "Выйти из игры", width = w, height = 1, bg = "white", fg = "black", command = close_game).grid(row = 5, column = 2)
